...
**Fetches 1yr daily price history per symbol** (1 extra API call/symbol/day, ~32 total/symbol/day alongside the options chain) to compute VRP (implied vs. realized vol). This was disabled for a while to save budget while options coverage was being extended to more symbols, then re-enabled once that coverage was established — the marginal cost turned out small (~3% more calls for a 24-symbol run) relative to leaving VRP Leaderboard / IV vs Realized Vol permanently blank for any symbol that's never had it. A price-history fetch failure is non-fatal (the options snapshot still saves); `vrp`/`realized_vol` just come back NaN for that symbol that day, same degraded-not-broken behavior as before, and the dashboard already shows a "needs price history" message for this case rather than a wrong number.
